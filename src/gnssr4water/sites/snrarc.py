...
class SNRArc(Arc):
    """
        A specialized arcs with SNR in Volts/Volts, and the sine of the elevation, possibly preprocessed for direct effects, atmosphere and bandpass filtering
    """

    def __init__(self,arc,noiseBandwidth=1,**kwargs):
        super().__init__(arc.prn,arc.system,arc.time,arc.elev,arc.az,arc.cnr0)
        
        #Apply preprocessing to input arc
        self.sinelev,self.snrv_v,self.noiseBandwidth=self.preprocess(arc,noiseBandwidth,**kwargs)
    ...
    def plot(self,ax=None,**kwargs):
        ...
        ax.plot(self.sinelev,self.snr,label=self.prn,**kwargs)
    ...
    @staticmethod
    def removePolyfit(sinelev,snrv_v,npoly=3):
        #remove direct signal as a polynomial fit
        x=sinelev-np.mean(sinelev)
        p=Polynomial.fit(x, snrv_v, npoly)
        res_snrv_v = snrv_v-p(x)
        return res_snrv_v
    
    @staticmethod
    def preprocess(arc,noiseBandwidth=1,**kwargs):
        ...
        if atmo_corr_tag in kwargs:
            ...
            sinelev=kwargs[atmo_corr_tag](arc.time,arc.elev)
        else:
            sinelev=np.sin(np.deg2rad(arc.elev))
         
        
        snrv_v=cnr0_2_vv(arc.cnr0,noiseBandwidth)

        if "npoly" in kwargs:
            #preprocess SNR by removing a polynomial fit from the data
            snrv_v=SNRArc.removePolyfit(npoly=kwargs['npoly'],sinelev=sinelev,snrv_v=snrv_v)
        elif "bandpass" in kwargs:
            sinelev,snrv_v=SNRArc.butterBandPass(bandpass=kwargs["bandpass"],sinelev=sinelev,snrv_v=snr_v_v,gnss_system=arc.system,direction=arc.direction)
        
        return sinelev,snrv_v,noiseBandwidth
    ...
    @staticmethod
    def butterBandPass(sinelev,snrv_v,gnss_system,direction,bandpass,butorder=3):
        ...
        lofreq=2*bandpass[0]/gnss_system.length
        hifreq=2*bandpass[1]/gnss_system.length
        sos = butter(butorder, [lofreq,hifreq], 'bandpass', fs=1/deltax, output='sos')
        
        if direction.startswith('desc'):
            filtered = sosfilt(sos, np.interp(sinelev_bp,sinelev[::-1],snrv_v[::-1]))
            sinelev_bp=sinelev_bp[::-1]
            snrv_v_bp=filtered[::1]
        else:
            filtered = sosfilt(sos, np.interp(sinelev_bp,sinelev,snrv_v))
            snrv_v_bp=filtered
